...
class AuthorProfileAPIView(APIView):
    ...
    def post(self, request):
        ...
        if Author.objects.filter(user=request.user).exists():
            return Response({"message": "Author profile already exists for this user"}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def put(self, request):
        ...
        try:
            ...
            return Response({"message": "Author profile does not exist for this user"}, status=status.HTTP_404_NOT_FOUND)
